...
class GererProfilEducateurPage extends StatefulWidget {
  const GererProfilEducateurPage({super.key});

  @override
  State<GererProfilEducateurPage> createState() => _GererProfilEducateurPageState();
}

class _GererProfilEducateurPageState extends State<GererProfilEducateurPage> {
  ...
  Future<void> _save() async {
    ...
    // Backend hook (optionnel) si endpoint dispo
    ...
    if (mounted) {
      ScaffoldMessenger.of(context).showSnackBar(
        const SnackBar(content: Text('Profil (Éducateur) enregistré.')),
      );
  ...
  Widget build(BuildContext context) {
    ...
    return Scaffold(
      appBar: AppBar(
        title: const Text('Gérer mon profil (Éducateur)'),
        actions: [
          IconButton(
        ...
        child: ListView(
          ...
          children: [
            ...
            TextFormField(
              ...
              decoration: const InputDecoration(
                labelText: 'Classe/Discipline (optionnel)',
                border: OutlineInputBorder(),
              ),
            ),
            const SizedBox(height: 12),
